...
enum IncidentsGrouper {
    ...
    static func sections(from incidents: [DisplayableIncident], sport: Sport) -> [IncidentSection] {
        ...
        for displayable in incidents {
            if displayable.incident.type == .periodEnd {
                ...
                sections.append(IncidentSection(id: sections.count, header: header, incidents: currentBucket))
                currentBucket = []
            } else {
    ...
    private static func makeHeader(
            ...
            case .basketball:
                            return marker.detail ?? AppStrings.quarter(index + 1)
        ...
        private static func makeHeader(forOngoingPeriodAt index: Int, sport: Sport) -> String {
            switch sport {
            case .football:
                return index == 0 ? AppStrings.firstHalf : AppStrings.secondHalf
            case .basketball, .americanFootball:
                return AppStrings.quarter(index + 1)
            }
        }

    private enum PeriodMarker {
        static let halfTime = "HT"
        static let fullTime = "FT"
    }

    private static func footballHeader(periodIndex: Int, marker: Incident) -> String {
        let baseName = periodIndex == 0 ? AppStrings.firstHalf : AppStrings.secondHalf
        guard let score = marker.score else { return baseName }
        switch marker.detail {
        case PeriodMarker.halfTime: return "\(AppStrings.halfTime) (\(score))"
        case PeriodMarker.fullTime: return "\(AppStrings.fullTime) (\(score))"
        default:                    return "\(baseName) (\(score))"
        }
    }

        private static func americanFootballHeader(periodIndex: Int, marker: Incident) -> String {
            let quarter = marker.detail ?? AppStrings.quarter(periodIndex + 1)
