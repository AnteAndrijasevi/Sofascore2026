...
final class EventDetailsViewController: UIViewController {
    ...
    private let homeTeamButton = UIButton(type: .custom)
    private let awayTeamButton = UIButton(type: .custom)
    ...
    @objc private func homeTeamTapped() {
        openTeamDetails(forHomeTeam: true)
    }

    @objc private func awayTeamTapped() {
        openTeamDetails(forHomeTeam: false)
    }

    private func openTeamDetails(forHomeTeam: Bool) {
        let team = forHomeTeam ? viewModel.homeTeam : viewModel.awayTeam
        let vm = TeamDetailsViewModel(team: team)
        let vc = TeamDetailsViewController(viewModel: vm)
    ...
    private func setupConstraints() {
        ...
        homeTeamButton.snp.makeConstraints { $0.edges.equalToSuperview() }
        awayTeamButton.snp.makeConstraints { $0.edges.equalToSuperview() }
    }
    
    private func setupActions() {
        homeTeamButton.addTarget(self, action: #selector(homeTeamTapped), for: .touchUpInside)
        awayTeamButton.addTarget(self, action: #selector(awayTeamTapped), for: .touchUpInside)
    }
    ...
    private func fetchImages() {
        viewModel.fetchImages { [weak self] home, away, league in
            guard let self else { return }
            homeLogoImageView.image = home
            awayLogoImageView.image = away
            titleLogoImageView.image = league
        }
    }
    ...
    private func setupContentSection() {
        if viewModel.isUpcoming {
            ...
            setupIncidentsSection()
        }
    }

    private func setupUpcomingSection() {
        let upcomingView = UpcomingEventView()
        upcomingView.onViewTournamentDetails = { [weak self] in
            self?.openLeagueDetails()
        }
        view.addSubview(upcomingView)
        upcomingView.snp.makeConstraints {
            $0.top.equalTo(teamsRow.snp.bottom)
            $0.leading.trailing.bottom.equalToSuperview()
        }
    }

    private func setupIncidentsSection() {
        let topGap = UIView()
        topGap.backgroundColor = AppColors.roundSectionBackground
        view.addSubview(topGap)
        topGap.snp.makeConstraints {
            $0.top.equalTo(teamsRow.snp.bottom).offset(16)
            $0.leading.trailing.equalToSuperview()
            $0.height.equalTo(8)
        }

        let incidentsView = IncidentsView(
            eventId: viewModel.eventId,
            sport: viewModel.sport,
            isLive: viewModel.isLive
        )
        let hosting = UIHostingController(rootView: incidentsView)
        ...
        hosting.view.snp.makeConstraints {
            $0.top.equalTo(topGap.snp.bottom)
            $0.leading.trailing.bottom.equalToSuperview()
        }
    }

    private func openLeagueDetails() {
        let leagueVM = viewModel.makeLeagueDetailsViewModel()
        let vc = LeagueDetailsViewController(viewModel: leagueVM)
        navigationController?.pushViewController(vc, animated: true)
    }
}
